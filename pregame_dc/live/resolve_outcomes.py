...
Reads the live bot's per-run `orders_summary_*.jsonl` logs, queries Gamma for
each game's resolved markets, fills in outcome and realised_pnl, and writes the
combined augmented log to `orders_summary_resolved.jsonl` (the inputs are never
modified).
...
With no --input it resolves every per-run order log in the logs directory; pass
--input to resolve one specific log.
"""
...
def _discover_inputs(output: Path) -> list[Path]:
    """Every live-bot order log in the logs directory, oldest first. Matches
    the per-run `orders_summary_<stamp>.jsonl` files (and any legacy
    `orders_summary.jsonl`); skips the resolved-output file itself."""
    return sorted(
        p for p in paths.LOGS_DIR.glob("orders_summary*.jsonl")
        if p != output and "resolved" not in p.name
    )
...
def main():
    ...
    ap.add_argument("--input", type=Path, default=None,
                    help="Resolve one specific orders log. Default: every "
                         "per-run orders_summary_*.jsonl in the logs directory.")
    ...
    inputs = [args.input] if args.input else _discover_inputs(args.output)
    inputs = [p for p in inputs if p.exists()]
    if not inputs:
        print("no orders logs found to resolve")
        return
    print(f"resolving {len(inputs)} order log(s): "
          f"{', '.join(p.name for p in inputs)}")
    ...
    print(f"inputs:         {len(inputs)} log file(s)")
