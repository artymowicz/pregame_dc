"""Pregame Dixon-Coles live trading bot.

At t = game_start + --time-seconds (default -10min), the bot:
...
import pregame_dc.polymarket.sdk as sdk
from pregame_dc.polymarket.sdk import UserWebSocket, jlog, now_str
from pregame_dc.polymarket.redundant_tracker import RedundantMarketTracker
from pregame_dc.discovery.soccer import build_constraints, fetch_markets
from pregame_dc.constants import MARKET_LABELS
from pregame_dc.discovery.slots import standard_slot_map
from pregame_dc.live.clob_v2 import build_signed_order_v2
from pregame_dc.models.dixon_coles import DixonColesModel
...
# The orders / kickoffs / ws-master log paths are NOT fixed constants: each
# bot run stamps them with its start time (see PregameDCBot.__init__) so a new
# run never overwrites or interleaves with an earlier run's logs.
DEFAULT_MODEL = paths.MODEL_DC_T_10MIN
...
class PregameDCBot:
    def __init__(self, args):
        ...
        sdk.order_log_path = str(self.ws_master_log)
    ...
    async def _ensure_user_ws(self):
        ...
        # to this run's ws_events_master log.
    ...
    async def fire_for_game(self, game_info):
        ...
        async with self._game_sem:
            ...
            with open(self.kickoffs_log, "a") as f:
